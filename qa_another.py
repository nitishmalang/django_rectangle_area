    ...
    try:
        with gp.Env(empty=True) as env:
            ...
            with gp.Model(env=env) as model:
                ...
                while removed > 0 or offset > 0:
                    ...
                    for i in indices:
                        ...
                        for j in range(n):
                            x[j].LB = lb[j]
                            x[j].UB = ub[j]
                            x[j].obj = objective_function_max[j]
                ...
                # Ensure that A_res and Aeq_res have the same number of columns
                num_cols_diff = Aeq_res.shape[1] - A_res.shape[1]
                if num_cols_diff > 0:
                    # Pad A_res with zeros to match the number of columns in Aeq_res
                    A_res = np.pad(A_res, ((0, 0), (0, num_cols_diff)), mode='constant')
