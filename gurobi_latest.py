...
def update_model(model, n, Aeq_sparse, beq, lb, ub, A_sparse, b_sparse, objective_vector):
    model.setMObjective(gp.LinExpr(objective_vector, model.getVars()), GRB.MINIMIZE)
    model.update()
    ...
    try:
        with gp.Env(empty=True) as env:
            ...
            with gp.Model(env=env) as model:
                ...
                for i in range(n):
                    objective_vector = A[i, :]
                    update_model(model, n, Aeq_sparse, beq, lb, ub, None, None, objective_vector)
                    model.optimize()
                    ...
                    objective_vector_max = -1 * np.asarray(objective_vector)
                    update_model(model, n, Aeq_sparse, beq, lb, ub, None, None, objective_vector_max)
                    model.optimize()
